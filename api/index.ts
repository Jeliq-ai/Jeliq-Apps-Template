...
import {
  ...
  removeExampleKeys,
  generateExpressOpenAPIAuthorizationSecurityHandlers,
} from '@core';
import {
  getBackend,
  getSupabaseClient,
...
initialize({
  ...
  securityHandlers: {
    Authorization: async (req, scopes, definition) => {
      return generateExpressOpenAPIAuthorizationSecurityHandlers(
        req,
        async (token) => verifyUserToken(token),
      );
    },
  },
});
